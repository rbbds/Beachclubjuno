  ...
  useEffect(() => {
    sanityClient
      ...
      .then(data => {
        ...
        if (r.menuUrl) setMenuUrl(r.menuUrl);
        if (r.photos?.length) {
          setPhotos(
            r.photos.slice(0, 4).map((p: any, i: number) => ({
              id: i + 1,
              src: p.image ? urlFor(p.image).width(900).url() : staticPhotos[i]?.src ?? '',
              alt: p.alt || staticPhotos[i]?.alt || '',
            }))
          );
        }
      })
